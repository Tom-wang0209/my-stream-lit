...
# 精细化比例分配：大幅拓宽第一列到 4.5 确保长模型名称完整处于单行内不出现折行换行
...
if st.session_state.show_guide:
    ...
    ```
    This will automatically download the model file (approximately 4.7GB for the first time, which may take some time depending on your network speed).
    
    ## Step 3: Ensure Ollama Service is Running
    By default, the Ollama service starts automatically after installation. To manually start or verify the service status:
    ...
    A: This is normal for local models. Consider upgrading your hardware or switching to cloud models for faster responses.
    """)
    st.info("Tip: Local models, while slower, are completely free and protect data privacy. Perfect for code review and learning!")
    st.stop()
...
with tab2:
    ...
    user_input_arch = st.chat_input("Describe your full-stack requirements...")
    if user_input_arch:
        st.session_state.messages.append({"role": "user", "content": user_input_arch})
        with st.chat_message("user"):
            st.markdown(user_input_arch)
