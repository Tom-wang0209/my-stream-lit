...
if 'captcha_num1' not in st.session_state:
    st.session_state.captcha_num1 = 0
if 'captcha_num2' not in st.session_state:
    st.session_state.captcha_num2 = 0
if 'captcha_op' not in st.session_state:
    st.session_state.captcha_op = '+'
if 'captcha_answer' not in st.session_state:
    st.session_state.captcha_answer = 0
...
def generate_captcha():
    ...
    if st.session_state.captcha_op == '+':
...
if not st.session_state.logged_in:
    ...
    st.markdown('<p style="color: #8b949e; text-align: center;">极客暗黑风 · 本地零成本运行</p>', unsafe_allow_html=True)
    ...
    with col2:
        ...
        # 人机验证
        generate_captcha()
        captcha_display = f"{st.session_state.captcha_num1} {st.session_state.captcha_op} {st.session_state.captcha_num2} = ?"
        captcha_input = st.text_input("人机验证", placeholder=captcha_display)
        ...
        with login_col:
            if st.button("登录", use_container_width=True):
                ...
                if username == "admin" and password == st.session_state.password and captcha_correct:
...
# ==================== 主界面 ====================
    ...
    st.markdown(f'<p style="color: #8b949e;">当前模型: {st.session_state.selected_model} | 语言: {st.session_state.selected_language}</p>',
                unsafe_allow_html=True)
