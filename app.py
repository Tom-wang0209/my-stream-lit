...
for key in ['logged_in', 'password', 'reset_count', 'conversations',
    ...
    if key not in st.session_state:
        st.session_state[key] = 'logged_in' if key == 'logged_in' else \
                                '123456' if key == 'password' else \
                                0 if key in ['reset_count', 'chat_tab', 'new_chat'] else \
                                [] if key in ['conversations', 'messages'] else \
                                None if key in ['current_conversation'] else \
                                'deepseek-chat' if key == 'selected_model' else 'black'
...
def analyze_code(code, language):
    prompt = f'''分析以下{language}代码，输出JSON：{{"highlights":[], "bugs":[{{"line":1,"issue":"","reason":""}}], "suggestions":[], "stats":{{"total_lines":0,"effective_lines":0,"empty_lines":0}}}}
代码：{code}'''
    try:
